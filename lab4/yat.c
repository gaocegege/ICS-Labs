...
{
	if(steps)
		sprintf(cmdbuf, "cd y86-app-bin; ../y86sim %s.bin %d > %s.sim", name,steps,name);
	else
		sprintf(cmdbuf, "cd y86-app-bin; ../y86sim %s.bin > %s.sim", name,name);
...
{
	if(steps)
		sprintf(cmdbuf, "cd y86-base; ./y86asm-base %s.ys; ./y86sim-base %s.bin %d > %s.sim.base", name,name,steps,name);
	else
		sprintf(cmdbuf, "cd y86-base; ./y86asm-base %s.ys; ./y86sim-base %s.bin > %s.sim.base",name,name,name);
...
{
    sprintf(cmdbuf, " diff ./y86-base/%s.sim.base ./y86-app-bin/%s.sim", name, name);
...
{
	if(steps){
    		sprintf(cmdbuf, "cd y86-ins-bin; ../y86sim %s.bin %d > %s.sim", name,steps,name);
	}
	else
		sprintf(cmdbuf, "cd y86-ins-bin; make %s.sim > /dev/null", name);
...
{
    sprintf(cmdbuf, " diff ./y86-base/%s.sim.base ./y86-ins-bin/%s.sim", name, name);
...
{
	if(steps)
		sprintf(cmdbuf, "cd y86-base; ./y86asm-base %s.ys; ./y86sim-base %s.bin %d",name,name,steps);
	else
		sprintf(cmdbuf, "cd y86-base; make %s.sim; cat %s.sim",name,name);
...
static void print_usage()
{
    printf("Usage: yat -c <name> [max_steps]\n"
		   ...
		   "              (e.g. yat -c prog9 4)\n"
           "  -s          test single instruction in ./y86-ins-bin/<name>.bin,\n"
           "              (e.g. yat -s rrmovl)\n"
           "  -S          test all instructions\n"
           "  -a          test single application in ./y86-app-bin/<name>.bin\n"
           "              (e.g. yat -a asum)\n"
...
static void clean_up()
{
    ...
    system("cd y86-ins-bin; make clean > /dev/null");
    system("cd y86-app-bin; make clean > /dev/null");
